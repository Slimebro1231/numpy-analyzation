...
# Perform curve fitting for the first 2.97 seconds
popt_1, pcov_1 = curve_fit(func, t_1, theta_1)

# Generate fitted curves
fit_1 = func(t_1, *popt_1)

# Plot the fitted curves for t <= 2.97
plt.plot(t_1, fit_1, linestyle='--', color='orange', label='Fitted Curve (t <= 2.97)')

# Display correlation coefficients and fit equations for t <= 2.97
...
# Plotting another line based on the equation y' = 1/2 * (-1120.5265) * t^2 + (the first theta value of the data)
first_theta = theta[0]
y_prime = 0.5 * (-1120.5265) * t_1**2 + first_theta
plt.plot(t_1, y_prime, linestyle='--', color='green', label="y' = 1/2 * (-1120.5265) * t^2 + θ_0")
