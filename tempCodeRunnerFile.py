...
gdp_file_path = 'gdpdata2.txt'  # Updated file path
...
# Use 2017 as the base year for the original data
base_year = 2017
base_year_deflator = gdp_data.loc[years == base_year, 'GDP_Deflator'].values[0]
print(f"Base year (original): {base_year}, Base year deflator: {base_year_deflator:.2f}")
...
# Plot the main Real GDP series
plt.figure(figsize=(10, 6))
plt.plot(years, real_gdp, label='Real GDP (Original)', linestyle='-', marker='o', color='blue')
plt.plot(years, gdp_data['Real_GDP_2000_Prices'], label='Real GDP (Constant 2000 Prices)', linestyle='--', marker='s', color='green')

# Label differences at each point
for i, year in enumerate(years):
    plt.text(year, gdp_data['Real_GDP_2000_Prices'].iloc[i] + 1000, 
             f"{gdp_data['Real_GDP_2000_Prices'].iloc[i]:.2f}", fontsize=8, color='red', ha='center')

plt.title('Real GDP: Original vs. Constant 2000 Prices', fontsize=16)
plt.xlabel('Year', fontsize=14)
plt.ylabel('Real GDP (Millions CAD)', fontsize=14)
plt.legend(fontsize=12)
plt.grid(True, linestyle='--', alpha=0.7)

plt.tight_layout()
plt.savefig('real_gdp_rebased_zoomed_plot.png', dpi=300, bbox_inches='tight')
plt.show()