...
t_3 = t[(t > 2.5) & (t <= 5.0)]  # Data after t = 5.0
theta_3 = theta[(t > 2.5) & (t <= 5.0)]
...
def func_3(t, angular_accel, initial_velocity, initial_angle):
    return 0.5 * angular_accel * t**2 + initial_velocity * t + initial_angle  # Changed sign for angular_accel for deceleration
...
popt_3, pcov_3 = curve_fit(func_3, t_3, theta_3, bounds=([angular_deceleration, -np.inf, initial_angle], [-angular_deceleration, np.inf, -initial_angle]))

# Generate fitted curve for the second range
fit_curve_2 = func_2(t_2, *popt_2)

fit_curve_3 = func_3(t_3, *popt_3)
...
plt.plot(t_3, fit_curve_3, linestyle='-', color='black', label='newplot', marker='o', markersize=1)
...
corrcoef_3 = np.corrcoef(theta_3, fit_curve_3)[0, 1]
eq_3 = r"$θ' = \frac{1}{2} \cdot %.4f \cdot t^2 + %.2f \cdot t + %.2f$" % (abs(popt_3[0]), popt_3[1], popt_3[2]) + '\n' + r"Correlation: %.8f" % corrcoef_3
plt.text(7.5, -300, eq_3, fontsize=10, color='red')
